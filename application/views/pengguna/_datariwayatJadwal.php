<script>
	...
	var material = {
		...
		DataFilter: ko.observableArray(['idUnix']),
		FilterValue: ko.observable('idUnix'),	 
		
	}
	material.back = function(tab){
		material.Mode('');
		material.grid.ajax.reload( null, false );
		$("input[name=idUnix]").attr("disabled", false);
		ko.mapping.fromJS(model.masterModel, material.Recordmaterial);
		model.activetab(tab);
	}
	material.selectdata = function(id) {
		ajaxPost("<?php echo site_url('master/Con_riwayatJadwal/getDataSelect') ?>", {id: id}, function (res) {
			material.back(0); /* */
			$("input[name=idUnix]").attr("disabled", true); 
			$("input[name=kodeKursus]").attr("disabled", true); 
			$("input[name=Siswa]").attr("disabled", true); 
			$("input[name=Tentor]").attr("disabled", true); 
			$("input[name=Umur]").attr("disabled", true); 
	...
	material.save = function(){
		...
		var url = "<?php echo base_url('master/Con_riwayatJadwal/save') ?>";
		if(material.Mode() === 'Update')
			url = "<?php echo base_url('master/Con_riwayatJadwal/update') ?>";
	...
	material.remove = function(id){
		...
		swal({
			...
			if (isConfirm) {
				ajaxPost("<?php echo base_url('master/Con_riwayatJadwal/delete') ?>", {id: id}, function (res) {
					material.back(1);
							...
							<div class="row p-t-23 " data-bind="with:Recordmaterial" >  
								<div class="col-md-6 margMin">
									<div class="form-group ">
										...
										<input type="text" name="kodeKursus" data-bind="value:kodeKursus, checkId: 'master/Con_riwayatJadwal/checkId'" id="" required="" class="form-control"> <!--  fungsi cek sebuah data yang di inputkan apakah sudah ada? -->
										<div class="form-control-feedback" data-bind="visible: model.CheckId()==true">ID Sama</div> <!-- jika sudah ada maka akan di tampilkan model.CheckId, menampilkan id Sama -->
									</div> 
								</div>  
								<div class="col-md-6 margMin">
									<div class="form-group ">
...
<script>
	$(document).ready(function () {
		...
		material.grid = $("#myTable").DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": "<?php echo base_url('master/Con_riwayatJadwal/getData') ?>",
				"type": "POST",
				"data": function(d){ 
			...
			{"data": "Umur"}, 
			{
				"data": "idUnix", /*-- isikan Id primary_key dari data yang di sebutkan di model --*/
