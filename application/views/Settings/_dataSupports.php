<script>
	...
	var material = {
		...
		DataFilter: ko.observableArray(['email']),
		FilterValue: ko.observable('email'),
	}
	...
	material.selectdata = function(id) {
		ajaxPost("<?php echo site_url('master/Con_supports/getDataSelect') ?>", {id: id}, function (res) {
			material.back(0);
	...
	material.save = function(){
		...
		var url = "<?php echo base_url('master/Con_supports/save') ?>";
		if(material.Mode() === 'Update')
			url = "<?php echo base_url('master/Con_supports/update') ?>";
							...
							<div class="row p-t-23 " data-bind="with:Recordmaterial" >  
								<div class="col-md-6 margMin">
									<div class="form-group ">
										...
										<input type="hidden" name="txtid" data-bind="value:id, checkData: 'Con_supports/checkData'" id="" required="" class="form-control"> 
...
<script>
	$(document).ready(function () {
		...
		material.grid = $("#myTable").DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": "<?php echo base_url('master/Con_supports/getData') ?>",
				"type": "POST",
				"data": function(d){ 
