<?php class Mo_pengguna extends CI_Model{
	...
	public function getFilterSiswa($q){
		$sql = "SELECT tbp.namaDepan as Siswa, tbp.namaBelakang as namaBelakangSiswa, tbp.penggunaId as idSiswa from tb_pengguna as tbp where tbp.tb_role_roleId = 3 and tbp.penggunaId LIKE '%" .$q. "%' LIMIT 10";
		$query = $this->db->query($sql); return $query->result();
	}
	...
	public function checkKtp($nomorKtp){
		...
		tb_pengguna.tb_kategori_kategoriId = tb_kategori.kategoriId and tb_pengguna.tb_role_roleId=2 and tb_pengguna.nomorKtp='$nomorKtp' ";
		$query = $this->db->query($sql);
