...
class Con_riwayatPembayaran extends CI_Controller
{
	function __construct(){
		parent::__construct();
		$this->load->helper(array('login','configsession')); cek_login();
		$this->load->model('master/Mo_riwayatPembayaran','mod'); 
	}
	function index(){
		$data['title'] = "Data Riwayat Pembayaran Siswa";
		// print session = $session['sessionName']; sessionname in configsession_helper file.
		$data['session']= session();
		$this->template->load('_template', 'pengguna/_datariwayatPembayaran', $data);
	}
	function getData(){
		...
		$res = $this->mod->getDatariwayatPembayaran($data); echo json_encode($res);
	}
